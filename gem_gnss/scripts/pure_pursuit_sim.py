...
# Integrate A* Planner
from hybrid_astar_planner.HybridAStar.hybrid_astar import planner
...
class PurePursuit(object):
    ...
    def start_pp(self):
        ...
        while not rospy.is_shutdown():
            ...
            if np.sqrt( (curr_x - (-21))**2 + (curr_y - (-17.5))**2) < 8 or self.reach_end:
                self.reach_end = True
                print("REACH END")
                if np.sqrt( (curr_x - (-21))**2 + (curr_y - (-17.5))**2) < 3:
                    self.ackermann_msg.speed = 0
                else:
                    self.ackermann_msg.speed = 0.8
                self.ackermann_msg.steering_angle = self.angle
                continue
                # return

            # Online Planning
            #############################################################
            # Save computational overhead, do not plan all the time
            if self.counter % 10 == 0:
                print("Current X: {}, Y: {}, Yaw: {}".format(curr_x, curr_y, curr_yaw))
                # start_time = time.time()
                # print(x[2], y[2], result_yaw[2])
                # print("Planner Time: ", time.time() - start_time)
                # print("calculated: ", x, y, result_yaw)
                # print("\n")
                try:
                    x, y, result_yaw = func_timeout(0.6, planner, (curr_x, curr_y, curr_yaw))
                    # x, y, result_yaw = planner(curr_x, curr_y, curr_yaw)
                    self.path_points_x = x
                    self.path_points_y = y
                    self.path_points_yaw = result_yaw
                    self.dist_arr = np.zeros(len(self.path_points_x))
            ...
            print(self.ackermann_msg.speed, self.ackermann_msg.steering_angle)

            # self.ackermann_msg = AckermannDrive()
            # self.ackermann_msg.speed          = 5
            # self.ackermann_msg.steering_angle = 0
            # self.ackermann_msg.acceleration   = 1
            # print(self.ackermann_msg)
