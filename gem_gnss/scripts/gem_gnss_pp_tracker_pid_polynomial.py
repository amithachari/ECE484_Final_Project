...
#################### Changed by Harry Starts Here ####################
            ...
            # # # Original codes above
            
            # my codes below
            # start -19, -9.5 ,45
            # polynomial of the path !!!!!!!!!! care about the parameters !!!!!!!!!!  Now this is in simulation frame
            # p_y = [-0.0263, -1.001, -27]
            p_y = [-0.0263, 0.0, 0.0]
            #polynomial_y = p_y[0] * cur_x ** 2 + p_y[1] * cur_x + p_y[2]
            
            # fix goal x position to figurer out goal y position
            x_goal = curr_x + (self.look_ahead -2)
            y_goal = p_y[0] * x_goal ** 2 + p_y[1] * x_goal + p_y[2]
            
            # finding the distance between the goal point and the vehicle
            # true look-ahead distance between a waypoint and current position
            L = self.dist((x_goal, y_goal), (curr_x, curr_y))
            
            slope = p_y[0]*2*x_goal + p_y[1] 
            lane_theta = math.atan(slope)
            alpha = lane_theta - curr_yaw
            
#################### Changed by Harry Ends=s Here ####################
            ...
            if(self.gem_enable == True):
                ...
                goal_x = 0.0
                goal_y = 0.0
                dist_goal = np.abs((curr_x - goal_x))
                print("Dist to goal_x",dist_goal)
                if dist_goal < 3.5:
                    self.brake_cmd.enable  = True
                    self.brake_cmd.clear   = False
                    self.brake_cmd.ignore  = False
