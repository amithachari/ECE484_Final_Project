...
class RRTStarDubins(RRTStar):
    ...
    def planning(self, animation=True, search_until_max_iter=True):
        ...
        """
        print("JJOJJOJ:SJGLSDJG:SJ:DGJ:SDJGGJSG")
...
class PurePursuit(object):
    ...
    def __init__(self):
        ...
        self.gnss_sub_loc = rospy.Subscriber("/novatel/inspva", Inspva, self.inspva_callback)
        self.lat = 0.0
        self.lon = 0.0
        self.heading = 0.0
    ...
    def start_pp(self):
        curr_x, curr_y, curr_yaw = 14.86, -0.7, np.deg2rad(0.0)
        # curr_x, curr_y, curr_yaw = self.get_gem_state()
        print("Subscribed once, not subscribing anymore, x, y, yaw:",curr_x, curr_y, curr_yaw)
        ...
        #     print("HIII")
        #     # connections = self.gnss_sub.get_num_connections()
        #     # print(connections,"CONNNN")
            
        #     # if connections > 0:
            # else:
                # self.rate.sleep()
            # rospy.loginfo("x, y, yaw::", curr_x, curr_y, curr_yaw)
...
def main():
    ...
    # /home/gem/Apollo_ws/src/vehicle_drivers/gem_gnss/scripts/xy_demo_online.csv
    # filename = os.path.join(dirname, 'xy_demo_online.csv')
    # print()
    rospy.init_node('gnss_pp_node_online', anonymous=True)
    pp = PurePursuit()
    start_x, start_y, start_yaw = 14.86, -0.7, np.deg2rad(0.0)
    try:
        start_x, start_y, start_yaw = pp.start_pp()
    ...
    goal = [-0.6, 0.7, np.deg2rad(190.0)]
    # start = [14.86, -0.7, np.deg2rad(0.0)]
    start = [start_x, start_y, start_yaw]
    rrtstar_dubins = RRTStarDubins(start, goal, max_iter=300, rand_area=[40.0, -10.0], obstacle_list=obstacleList)
    ...
    path.reverse()
    with open('/home/gem/Apollo_ws/src/vehicle_drivers/gem_gnss/scripts/xy_demo_online.csv', 'w') as file:
